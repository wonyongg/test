...
/**
 * @author wonyonghwang
 * SAX(Simple API for XML) 파싱 방식을 이용하는 방법으로 대용량 엑셀 파일을 다뤄도 OOM이 일어나지 않게 한다.(나눠서 하기 처리하기 때문)
 */
@Slf4j
public class ExcelSheetHandler implements XSSFSheetXMLHandler.SheetContentsHandler {
    ...
    // 실제 데이터가 시작되는 Row Number의 인덱스 값. ex) 데이터가 5행부터 시작이면 -1한 값인 4

    private int currentRowNum = startRowNum - 1;
    // 현재 읽고 있는 cell의 Row Number로, 실제 데이터가 시작되는 RowNumber에서 -1을 해 초기값으로 header를 가리키게 한다. ex) 5 -> 4
    ...
    public void startRow(int currentRowNum) {
        this.checkedCol = -1; // cell 메서드의 마지막에 checkedCol은 마지막 열의 인덱스 값이 들어가니 -1로 다시 초기화하는 것.
    ...
    public void endRow(int currentRowNum) {
        ...
        } else { // 만약 현재 행이 0이 아닌 경우 rows 목록에 데이터(row, 한 행에 들어있는 데이터 리스트)를 추가
